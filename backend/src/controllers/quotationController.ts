...
export const updateQuotationStatus = async (req: Request, res: Response) => {
  try {
    console.log(
      "🔍 updateQuotationStatus called with body:",
    ...
    console.log(
      `📦 Found order: ${order_id}, type: ${order.type}, status: ${order.status}`,
    );
    ...
    if (order.type === "purchase") {
      if (order.status === "completed") {
        return res
          .status(400)
          .json({ error: "Order already marked as completed" });
      }

      if (updated_products && Array.isArray(updated_products)) {
        products = products.map((p: any) => {
          const match = updated_products.find(
            (u: ProductUpdate) => u.product_code === p.product_code,
          );
          if (!match) return p;
        ...
        const allReceived = products.every(
          (p: any) => p.quantity_ordered === p.quantity_received,
        );
    ...
    return res
      .status(400)
      .json({ error: "Unrecognized order type or transition" });
  } catch (err) {
    console.error(err);
